...
`cli.NewRootCmd` prend maintenant un paramètre `version string` et le pose sur `root.Version` - Cobra enregistre `--version` automatiquement dès que ce champ est non vide, avec un format de sortie standard (`lynxor version <X>`). Pas de sous-commande `version` ni de flag custom à maintenir.
...
- Vérifié empiriquement, pas seulement lu dans la doc Cobra : `go build -o lynxor . && ./lynxor --version` → `lynxor version dev` ; avec `-ldflags "-X main.version=v1.0.0"` → `lynxor version v1.0.0` ; `go install -ldflags "..." .` confirmé aussi (syntaxe valide, testée en local avec un `GOBIN` de scratch).
