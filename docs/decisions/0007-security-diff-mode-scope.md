...
`lynxor diff main feature-branch` (vision.md Phase 3) doit montrer ce qu'une branche/PR introduit ou corrige, pas un score statique du repo entier. Deux questions tranchées avec l'utilisateur avant l'implémentation : comment apparier un finding entre les deux refs pour décider NEW/FIXED/inchangé, et sur quoi baser le code de sortie.
