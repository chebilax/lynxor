  ...
  script:
    - |
      if [ "$CI_PIPELINE_SOURCE" = "merge_request_event" ]; then
        ...
        lynxor diff "$CI_MERGE_REQUEST_DIFF_BASE_SHA" "$head_sha"
      else
        lynxor scan . --format json | tee lynxor-report.json
      fi
  artifacts:
    when: always
    paths:
    ...
    steps {
        ...
        script {
            if (env.CHANGE_ID) {
                ...
                sh "lynxor diff ${baseSha} ${env.GIT_COMMIT}"
            } else {
                sh 'lynxor scan . --format json | tee lynxor-report.json'
            }
        }
    }
    post {
        always {
            archiveArtifacts artifacts: 'lynxor-report.json', allowEmptyArchive: true
        }
    }
}
```
