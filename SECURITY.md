...
Lynxor scans other people's repositories for security problems, so a vulnerability in Lynxor itself deserves at least the same seriousness it asks of everyone else's code.

## Supported versions

Lynxor is pre-1.x-stable and mono-maintained. Only the latest tagged release and `main` are supported — there is no long-term-support branch yet.
...
- Findings from `--deps` about Lynxor's own third-party dependencies (`go.mod`/`go.sum`) — please open a regular issue or PR; these get the same triage as any other dependency update.
