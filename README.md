...
*This README is everything you need to use Lynxor. [`docs/`](docs/) explains the* why *behind each design decision — worth a look if you're curious or contributing, not required just to run the tool.*
...
```bash
./lynxor scan .
./lynxor scan /path/to/other/repo
```
...
```bash
./lynxor scan . --full-history   # no time budget: walk all reachable history, plus dangling commits from deleted branches
./lynxor scan . --no-history     # skip history entirely, working tree only
```
...
```bash
./lynxor scan . --deps   # check pinned dependencies against known vulnerabilities (requires network)
```
...
```bash
./lynxor scan . --format json
```

See [docs/decisions/0009-json-output-schema.md](docs/decisions/0009-json-output-schema.md) for the schema and why it's versioned separately from Lynxor's internal Go types.
...
```bash
./lynxor scan . --format html > report.html
```
...
`lynxor diff` shows only what changed between two git refs — built for a pull request, where "what did this PR introduce or fix" matters more than a static score for the whole repo:
...
```bash
./lynxor scan . --plugin /path/to/your-plugin
./lynxor scan . --plugin /path/to/plugin-a --plugin /path/to/plugin-b   # --plugin is repeatable
```
...
A `go.sum` pinning an old version with known CVEs, checked with `--deps` (this dependency has 4 distinct known vulnerabilities after deduplication — OSV tracks the same issue under multiple ID schemes, e.g. both a GHSA and a PYSEC id, and Lynxor collapses those aliases so the same real vulnerability isn't scored twice):
...
`lynxor diff main feature-branch` on a branch that fixes an old Dockerfile issue but introduces a secret:
...
Phase 3 — dependency vulnerability scanning for `go.sum` and `requirements.txt` against OSV.dev, opt-in via `--deps` (the only network-dependent check Lynxor has; the default scan stays 100% local and deterministic — see `docs/decisions/0004-dependency-scanner-network.md`); and Security Diff Mode (`lynxor diff <ref-a> <ref-b>`), which reuses the same secrets/Docker/CI/CD rules against two git refs read directly from git (no checkout) and reports only what changed.
...
Found a security issue in Lynxor itself (not a false positive/negative in a detection rule — see [SECURITY.md](SECURITY.md) for that distinction)? See [SECURITY.md](SECURITY.md) for how to report it privately.
